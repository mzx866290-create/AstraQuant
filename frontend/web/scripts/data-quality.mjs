...
assert.equal(dataQuality.formatConfidenceLabel('low'), '低')
assert.equal(dataQuality.formatQualitySource('unavailable'), '暂不可用')
assert.equal(dataQuality.formatQualitySource('database+eastmoney'), '本地数据库/东方财富')
assert.equal(dataQuality.formatQualitySource('AKShare/东方财富'), 'AKShare/东方财富')
