...
async function editQuota(user: any) {
  const daily = prompt('设置每日配额:', String(user.daily_limit))
  const monthly = prompt('设置每月配额:', String(user.monthly_limit))
...
async function resetQuota(user: any) {
  if (!confirm(`确定重置用户 "${user.username}" 的配额吗?`)) return
  try {
