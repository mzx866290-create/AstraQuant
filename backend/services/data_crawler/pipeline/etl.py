...
class DragonTigerETL:
    """Clean dragon tiger rows and write them to ClickHouse."""

    async def save(self, data: list[dict], source: str) -> int:
        if not data:
            logger.warning("no dragon tiger data to write")
            return 0

        cleaned_by_key = {}
        for item in data:
            row = self._clean_row(item, source)
            if row:
                cleaned_by_key[(row["trade_date"], row["symbol"], row["reason"])] = row

        cleaned = list(cleaned_by_key.values())
        if not cleaned:
            logger.warning("no valid dragon tiger rows after cleaning")
            return 0

        await self._batch_write(cleaned)
        logger.info("wrote %s dragon tiger rows", len(cleaned))
        return len(cleaned)

    def _clean_row(self, item: dict, source: str) -> Optional[dict]:
        code = self._normalize_code(self._pick(item, "代码", "股票代码", "证券代码", "symbol", "ts_code"))
        if not code:
            return None

        try:
            return {
                "trade_date": self._parse_date(self._pick(item, "上榜日", "交易日期", "trade_date", "date")),
                "symbol": code,
                "name": str(self._pick(item, "名称", "股票简称", "证券简称", "name") or ""),
                "reason": str(self._pick(item, "上榜原因", "上榜理由", "reason") or ""),
                "close_price": self._safe_float(self._pick(item, "收盘价", "close", "close_price")),
                "change_pct": self._safe_float(self._pick(item, "涨跌幅", "涨跌幅(%)", "pct_chg", "change_pct")),
                "turnover": self._safe_float(self._pick(item, "龙虎榜成交额", "成交额", "turnover")),
                "buy_amount": self._safe_float(self._pick(item, "龙虎榜买入额", "买入额", "buy_amount")),
                "sell_amount": self._safe_float(self._pick(item, "龙虎榜卖出额", "卖出额", "sell_amount")),
                "net_amount": self._safe_float(self._pick(item, "龙虎榜净买额", "净买额", "net_buy", "net_amount")),
                "buy_seats": self._json_text(self._pick(item, "买入席位", "buy_seats")),
                "sell_seats": self._json_text(self._pick(item, "卖出席位", "sell_seats")),
                "source": source,
            }
        except (TypeError, ValueError) as exc:
            logger.debug("failed to parse dragon tiger row: %s", exc)
            return None
    ...
    async def _batch_write(self, rows: list[dict]) -> int:
        ...
        rows = _with_write_metadata(rows)
        table = _clickhouse_table_name("CLICKHOUSE_DRAGON_TIGER_TABLE", "dragon_tiger")
        columns = [
            "trade_date",
            "symbol",
            "name",
        ...
        return _write_rows_to_clickhouse(
            ...
            columns=columns,
            rows=rows,
            key_columns=["trade_date", "symbol", "reason"],
            row_label="dragon tiger",
        )
